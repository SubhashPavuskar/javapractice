package com.tecnotree.home.practice.algorithm;

import java.util.Arrays;

public class PrintArrayPairsQuickSort
{
	...
	public static void printPairsUsingTwoPointers(int[] numbers, int k)
	{
		...
		while (left < right)
		{
			...
			if (sum == k)
			{
				...
				left = left + 1;
				right = right - 1;
				System.out.println("sum == k so :: "+left+"---"+right);
			}
			else if (sum < k)
			{
				left = left + 1;
				System.out.println("sum < k so left:: "+left);
			}
			else if (sum > k)
			{
				right = right - 1;
				System.out.println("sum > k so right:: "+right);
			}
		}
	}
